...
# omanomad Installation Script (Arch/Omarchy port)
#
# Arch port of Project NOMAD's Installation Script v1.0.0 by Crosstalk
# Solutions, LLC (https://crosstalksolutions.com), adapted for
# Arch-based systems (Omarchy). Function-by-function port: control flow,
# prompts, secrets handling, compose customization, helper downloads,
# container startup, and GPU verification intentionally mirror upstream.
#
# Run with elevated privileges; the omanomad panel launches this via
# `pkexec bash bin/install.sh` in a floating terminal (pkexec, not sudo,
# because there is no terminal for a password prompt otherwise).
#
# No firewall rules are managed here: stock Arch/Omarchy has no default
...
# Pinned upstream sources, image digests, and the verified fetcher live in
# bin/lib/preflight.sh so install.sh (fresh install) and update.sh
# (legacy-compose heal) share one copy of each trust value.
...
ensure_dependencies_installed() {
  ...
  # Check for gpg (Arch package: gnupg; used only if the NVIDIA path needs key handling)
  if ! command -v gpg &> /dev/null; then
    missing_pkgs+=("gnupg")
  fi

  # Check for lspci (Arch package: pciutils; used for GPU detection)
...
ensure_docker_installed() {
  if ! command -v docker &> /dev/null; then
    ...
    # Arch: Docker and Compose come from the official repos — never fetch
    # or execute upstream's remote convenience install script.
    ...
    # pacman does not enable services, but NOMAD expects Docker running at
    # boot (its containers are restart: unless-stopped).
...
setup_nvidia_container_toolkit() {
  ...
  # Arch: a single official-repo package — no GPG keyring or apt source setup.
  if ! sudo pacman -S --needed --noconfirm nvidia-container-toolkit 2>/dev/null; then
...
# NOTE: upstream's trailing commented-out free_space_check() is intentionally
# not ported — dead code referencing another project's eus_dir/cancel_script.
